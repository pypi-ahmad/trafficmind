...
- **Dismiss** a violation — Two-step action: click Dismiss, then confirm the warning prompt before the review is submitted. Requires the operator's name. Optional review note.

Both actions call `POST /api/v1/violations/{id}/review` and revalidate the page. The operator's name is persisted in the browser's `localStorage` and pre-filled on subsequent reviews. Violations already in a terminal state (confirmed or dismissed) do not show action buttons.
...
**Actions** (shown per alert row based on current status):
- **Acknowledge** — Available on New and Escalated alerts. Records the operator's name and an optional note. Calls `POST /api/v1/alerts/{id}/acknowledge`.
- **Resolve** — Available on New, Acknowledged, and Escalated alerts. Records the operator's name and an optional note. Calls `POST /api/v1/alerts/{id}/resolve`.

The operator's name is shared with the Cases review flow via the same `localStorage` key and is pre-filled automatically. Alerts already in Resolved or Suppressed state display no action buttons. Suppress and escalate operations are available at the API level but are not exposed in the frontend.

### Cameras

**Route:** `/cameras`

Lists all registered cameras with status badges, stream counts, and calibration timestamps. Selecting a camera navigates to the Camera Detail page.

If the camera service cannot be reached, a recovery prompt is shown with a "Try again" link.
...
- **Access Control Policy** — Lists role/permission assignments for the active policy, including per-role permission sets and protected action requirements. Read-only for operators; configuration is managed by administrators.
- **Evidence Privacy and Access** — Preview of evidence access permissions for the current policy role, showing which permissions are effective and explanatory notes.

Admin Tools links provide quick navigation to Evaluation and Reports.
...
- **Getting Around** — Navigation descriptions with direct links to each page (Dashboard, Cases, Alerts, Cameras, Reports, Evaluation, Settings).
- **Glossary** — Definitions for: Violation, Detection, Case, Alert, Zone, Stream, Junction, Escalation, Export, Severity, Precision, Recall, IoU, Continuity Score, ID Switch, Benchmark Test.
...
- **Alert suppress/escalate not in UI.** The backend supports suppress (`POST /api/v1/alerts/{id}/suppress`) and escalate (`POST /api/v1/alerts/{id}/escalate`) operations on alerts, but these are not exposed in the frontend. Acknowledge and Resolve are available.
- **Export creation not in UI.** The backend supports creating case export bundles (`POST /api/v1/exports`), but the frontend reports page only lists existing exports.
